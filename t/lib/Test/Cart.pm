...
sub run_tests {
    ...
    subtest 'main cart tests' => sub {
        ...
        qr/Product with sku .+ does not exist/,
          "add sku that does not exist in db";
        ...
        cmp_ok( $ret->[0]->canonical_sku,
            'eq', 'os28085', "Check canonical_sku" );
        ...
        isa_ok( $ret->[0], 'Interchange6::Cart::Product' );
        cmp_ok( $ret->[0]->sku, 'eq', 'os28005',
            "Check sku of returned product" );
        ...
        cmp_ok( sprintf( "%.2f", $ret->[0]->selling_price ),
            '==', 8.99, "Check price of returned product" );
        cmp_ok( $ret->[0]->quantity,
            '==', 1, "Check quantity of returned product is 1" );
        ...
        cmp_ok( sprintf( "%.2f", $ret->[0]->selling_price ),
            '==', 8.99, "Check price of returned product" );
        cmp_ok( $ret->[0]->quantity,
            '==', 2, "Check quantity of returned product is 2" );
        cmp_ok( $cart->count, '==', 1,
            "Check number of products in the cart is 1" );
        cmp_ok( sprintf( "%.2f", $cart->subtotal ),
        ...
        cmp_ok( $ret->[0]->quantity,
            '==', 10, "Check quantity of returned product is 10" );
        cmp_ok( sprintf( "%.2f", $ret->[0]->total ),
            '==', 84.90, "Check total of returned product is 84.90" );
        cmp_ok( $cart->count, '==', 1,
            "Check number of products in the cart is 1" );
        cmp_ok( sprintf( "%.2f", $cart->subtotal ),
        ...
        cmp_ok( $ret->[0]->quantity,
            '==', 2, "Check quantity of returned product is 2" );
        cmp_ok( sprintf( "%.2f", $ret->[0]->total ),
            '==', 49.98, "Check total of returned product is 49.98" );
        cmp_ok( $cart->count, '==', 2,
        ...
        cmp_ok( $cart->count, '==', 1,
            "cart count after update of os28006 to 0." );
        ...
        cmp_deeply(
            ...
            [
                superhashof(
                    {
                        level   => "debug",
                        message => "carts_var_name: ic6_carts"
                    }
                ),
                superhashof(
                    {
        ...
        lives_ok( sub { $cart = cart("test") },
            "get new TestCart with name test" );
        ...
        lives_ok( sub { $cart->test_attribute("foobar") },
            "set test_attribute" );
